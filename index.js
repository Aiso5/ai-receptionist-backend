...
app.post('/handle-confirmation', async (req, res) => {
  console.log('🔔 /handle-confirmation hit:', { headers:req.headers, body:req.body });
  try {
    const confirmation = (req.body.confirmation||'').trim().toLowerCase();
    const apptId       = req.body.metadata?.appointmentId;
    if (!apptId) {
    ...
    console.log(`✔️ Appointment ${apptId} set to ${newStatus}`);
    res.sendStatus(200);
  } catch(err) {
