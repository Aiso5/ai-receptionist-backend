...
const {
  BASE_URL,
  BLAND_API_KEY,
  GHL_CALENDAR_ID,   // your single Event calendar
  GHL_API_KEY,       // v1 key
  TWILIO_ACCOUNT_SID,
  TWILIO_AUTH_TOKEN,
  TWILIO_PHONE_NUMBER
...
// ─── MAP SERVICES → appointmentTypeId ─────────────────────────────────
...
app.post('/check-and-book', async (req,res) => {
  try {
    ...
    // Build v1 event‑calendar payload
    const payload = {
  ...
  try {
    if (['no-answer','busy'].includes(status)) {
      await twilioClient.messages.create({
        from:TWILIO_PHONE_NUMBER, to:phone_number,
