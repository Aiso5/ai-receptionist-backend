...
// Map services → Appointment Type IDs
...
app.post('/check-and-book', async (req, res) => {
  try {
    ...
    // Fetch slots
    const slotsRes=await axios.get('https://rest.gohighlevel.com/v1/appointments/slots',{
    ...
    await axios.post('https://rest.gohighlevel.com/v1/appointments/',payload,
...
app.post('/send-reminders',async(req,res)=>{
  ...
  }catch(err){console.error('Reminder error:',err.response?.data||err);res.status(500).send('Failed to send reminders');}
});
...
app.post('/handle-confirmation',async(req,res)=>{
  ...
  try{
    const{startMs,endMs}=getTomorrowRange();
    const resList=await axios.get('https://rest.gohighlevel.com/v1/appointments/',{
      headers:{Authorization:`Bearer ${GHL_API_KEY}`},
      params:{startDate:startMs,endDate:endMs,calendarId:GHL_CALENDAR_ID,includeAll:true}
    });
    const appts=resList.data.appointments||[];
    console.log('Appointments fetched for confirmation:',appts);
    const appt=appts.find(a=>a.phone===phone_number||(a.contact&&a.contact.phone===phone_number));
...
app.post('/call-status',async(req,res)=>{
  ...
  try{if(['no-answer','busy'].includes(status))await twilioClient.messages.create({from:TWILIO_NUMBER,to:phone_number,body:'We tried calling to confirm your appointment tomorrow. Reply YES, NO, or RESCHEDULE.'});res.sendStatus(200);}catch(err){console.error('Call-status error:',err);res.status(500).send('Call status processing failed');}
});
